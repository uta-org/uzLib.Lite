...
public static class GuiHelper
{
    ...
    public static void DrawLine(Vector2 lineStart, Vector2 lineEnd, Texture2D texture, int thickness)
    {
        ...
        var drawingRect = new Rect(lineStart.x,
            lineStart.y - thicknessOffset,
        ...
        {
            var drawingRectWidth = Mathf.RoundToInt(drawingRect.width);
            var drawingRectHeight = Mathf.RoundToInt(drawingRect.height);

            for (var y = 0; y < drawingRectHeight; y += texture.height)
            for (var x = 0; x < drawingRectWidth; x += texture.width)
