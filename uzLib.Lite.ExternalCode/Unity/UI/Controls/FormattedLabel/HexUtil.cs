...
namespace UnityEngine.UI.Controls
{
    public static class HexUtil
    {
        ...
        public static int HexToInt(string value)
        {
            ...
            try
            {
                intValue = int.Parse(value, NumberStyles.HexNumber);
            }
            catch (Exception)
            {
                intValue = 0;
        ...
        public static bool HexToColor(string value, out Color color)
        {
            ...
            var r = HexToInt(rText) / 255.0f;
            var g = HexToInt(gText) / 255.0f;
            var b = HexToInt(bText) / 255.0f;
            var a = HexToInt(aText) / 255.0f;
            if (r < 0 || g < 0 || b < 0 || a < 0)
            {
                color = Color.white;
                return false;
